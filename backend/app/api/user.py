...
async def update_profile(
    update_data: UserUpdate, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)
):
    """更新用户个人资料"""
    # 只允许更新用户名和邮箱
    update_dict = {}
...
async def get_usage(current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)):
    """
    获取当前用户的使用量信息
    包括等级、今日对话/生图使用量和剩余量
    """
...
async def check_chat_limit(current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)):
    """
    检查用户是否可以发送对话请求
    前端在发送消息前调用此接口检查
    """
    can_send, error_msg = await usage_service.check_chat_limit(db, current_user)
    return {"allowed": can_send, "message": error_msg if not can_send else ""}


@router.get("/usage/check-image")
async def check_image_limit(current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)):
    """
    检查用户是否可以发送生图请求
    前端在调用绘图服务前调用此接口检查
    """
