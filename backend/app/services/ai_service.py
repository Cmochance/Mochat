    ...
    for url in image_urls:
        try:
            ...
            if image_data:
                ...
                logger.info(
                    f"[Vision] 图片已转换为 Base64: {validated_url[:50]}... ({len(image_data['base64'])} chars)"
                )
            else:
                # 下载失败，尝试使用原始 URL（可能某些 API 支持）
                logger.warning(f"[Vision] 图片下载失败，使用原始 URL: {validated_url}")
                image_part = {"type": "image_url", "image_url": {"url": validated_url}}
                parts.append(image_part)
...
async def convert_messages_for_vision(messages: list[dict]) -> list[dict]:
    ...
    for msg in messages:
        ...
        if role == "user" and isinstance(content, str):
            ...
            converted.append({"role": role, "content": parsed_content})
        else:
            # 助手消息或已处理的消息保持原样
    ...
    async def chat_stream(
        ...
        try:
            ...
            response = await self.client.chat.completions.create(
                model=use_model, messages=chat_messages, stream=True, temperature=temperature, max_tokens=max_tokens
            )
