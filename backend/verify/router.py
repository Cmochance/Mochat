...
async def send_verification_code(
    ...
    success, message, cooldown = await VerificationService.send_code(
        db, email=request.email, purpose=request.purpose, ip=ip
    )
    ...
    return SendCodeResponse(
        success=True, message=message, expires_in=config.CODE_EXPIRE_MINUTES * 60, cooldown=cooldown
    )
...
async def get_cooldown(
    ...
    if purpose not in config.VALID_PURPOSES:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="无效的验证用途")

    cooldown = await VerificationService.get_cooldown(db, email, purpose)

    return {"email": email, "purpose": purpose, "cooldown": cooldown, "can_send": cooldown == 0}
