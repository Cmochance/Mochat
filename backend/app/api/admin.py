...
from ..schemas.user import TierUpdateRequest, UserAdminResponse, UserResponse, UserUpdate
...
from ..services.usage_service import TIER_LIMITS, usage_service
...
async def toggle_user_status(
    user_id: int, current_user: User = Depends(get_admin_user), db: AsyncSession = Depends(get_db)
):
    """切换用户状态（启用/禁用）"""
...
async def set_config(
    key: str, data: ConfigUpdate, current_user: User = Depends(get_admin_user), db: AsyncSession = Depends(get_db)
):
    """设置系统配置"""
...
async def add_keyword(
    data: KeywordCreate, current_user: User = Depends(get_admin_user), db: AsyncSession = Depends(get_db)
):
    """添加限制词"""
    keyword = data.keyword.strip()
    if not keyword:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="关键词不能为空")

    result = await crud.add_keyword(db, keyword, current_user.id)
    await db.commit()

    if not result:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="关键词已存在")
...
async def delete_keyword(
    keyword_id: int, current_user: User = Depends(get_admin_user), db: AsyncSession = Depends(get_db)
):
    """删除限制词"""
...
@router.post("/models")
async def add_model(
    data: ModelCreate, current_user: User = Depends(get_admin_user), db: AsyncSession = Depends(get_db)
):
    """添加允许的模型"""
    ...
    result = await crud.add_allowed_model(
        db, model_id, data.display_name.strip() if data.display_name else None, data.sort_order
    )
    await db.commit()

    if not result:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="该模型已存在")
...
async def delete_model(
    model_db_id: int, current_user: User = Depends(get_admin_user), db: AsyncSession = Depends(get_db)
):
    """删除允许的模型"""
...
async def update_user_tier(
    ...
    if data.tier not in ["free", "pro", "plus"]:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="无效的用户等级，必须是 free、pro 或 plus")
    ...
    if target_user.role == "admin":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="无法修改管理员的等级")
...
@router.get("/tiers")
async def get_tier_info(current_user: User = Depends(get_admin_user)) -> Dict[str, Any]:
    """获取所有等级配置信息"""
    return {
        "tiers": [
            {
...
async def export_usage_events(
    ...
    filename = f"usage_events_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}.csv"
    return Response(
        content=csv_data,
