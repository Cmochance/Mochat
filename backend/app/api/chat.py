...
# 自定义模板路径
CUSTOM_REFERENCE_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "custom_reference.docx"
)
...
async def get_models(current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)):
    ...
    if allowed_models:
        ...
        for allowed in allowed_models:
            ...
            for m in all_models_data:
                ...
                models.append(
                    ModelInfo(id=allowed.model_id, name=allowed.display_name or allowed.model_id, owned_by=None)
                )
    else:
        # 如果数据库中没有配置，返回所有模型（后向兼容）
        models = [ModelInfo(**m) for m in all_models_data]

    return ModelsResponse(models=models, default_model=ai_service.default_model)
...
async def create_session(
    request: SessionCreate, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)
):
    """创建新会话"""
...
async def get_session(
    session_id: int, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)
):
    """获取会话详情"""
...
async def delete_session(
    ...
    if not success:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="会话不存在或无权删除")
    return {"message": "删除成功"}
...
async def get_messages(
    ...
    """
    result = await chat_service.get_session_messages_paginated(db, session_id, current_user, limit, before_id)
    if result is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="会话不存在")
    return result
...
async def save_message(
    request: MessageCreate, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)
):
    """
    ...
    message = await crud.create_message(db, request.session_id, request.role, request.content, request.thinking)
    return message
...
async def generate_image(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="无法连接到图像生成微服务，请确保微服务已启动并正常运行",
        )
    except Exception as exc:
        error_code = "image_gateway_exception"
...
async def generate_ppt(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="无法连接到 PPT 生成微服务，请确保微服务已启动并正常运行",
        )
    except Exception as exc:
        error_code = "ppt_gateway_exception"
...
async def regenerate_response(
    ...
    async def generate():
        async for chunk in chat_service.regenerate_response(db, session_id, current_user):
            yield f"data: {json.dumps(chunk, ensure_ascii=False)}\n\n"
            # 强制事件循环立即处理，确保数据被发送
            await asyncio.sleep(0)
...
async def export_to_docx(request: ExportRequest, current_user: User = Depends(get_current_active_user)):
    """
